...
if(isset($_POST['update'])){
   ...
   $update_profile = $koneksi->prepare("UPDATE `users` SET name = ?, email = ? WHERE id = ?");
   ...
   if(!empty($image)){
      ...
      if($image_size > 2000000){
         ...
         $update_image = $koneksi->prepare("UPDATE `users` SET image = ? WHERE id = ?");
   ...
   if(!empty($previous_pass) || !empty($new_pass) || !empty($confirm_pass)){
      if($previous_pass != $old_pass){
         ...
         $update_password = $koneksi->prepare("UPDATE `users` SET password = ? WHERE id = ?");
